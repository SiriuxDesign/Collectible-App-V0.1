          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div className="flex gap-2">
                <select
                  value={bulkEditField || ''}
                  onChange={(e) => handleBulkFieldChange(e.target.value as 'status' | 'type' | 'rarity')}
                  className="focusable-element border border-gray-300 dark:border-gray-600 rounded px-3 py-2 text-sm bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200"
                >
                  <option value="" className="dark:text-gray-300">Select field to edit</option>
                  <option value="status" className="dark:text-gray-300">Status</option>
                  <option value="type" className="dark:text-gray-300">Type</option>
                  <option value="rarity" className="dark:text-gray-300">Rarity</option>
                </select>
                ...
                {bulkEditField && (
                  <select
                    ...
                    className="focusable-element border border-gray-300 dark:border-gray-600 rounded px-3 py-2 text-sm bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200"
                  >
                  <option value="" className="dark:text-gray-300">Select new value</option>
                  {bulkEditField === 'status' && statusOptions
                  ...
                  {bulkEditField === 'type' && typeOptions.map(option => (
                    <option key={option} value={option} className="dark:text-gray-300">{option}</option>
                  ))}
                  {bulkEditField === 'rarity' && rarityOptions.map(option => (
                    <option key={option} value={option} className="dark:text-gray-300">{option}</option>
                  ))}
                </select>
              )}
              ...
              {bulkEditField && bulkEditValue && (
                <button
                  ...
                  className="focusable-element px-3 py-2 text-sm font-medium text-white bg-[#d2001e] dark:bg-red-700 rounded hover:bg-red-700 dark:hover:bg-red-800 transition-colors"
                >
                  Apply Changes
                </button>
              )}
